...
export default class {
	...
	handleEditTicket(e, bill, bills) {
		const isOpen = this.openBillId === bill.id;
	...
	handleShowTickets(e, bills, index) {
		this.sectionsOpen[index] = !this.sectionsOpen[index];
		this.openBillId = null;

		const $arrow = $(`#arrow-icon${index}`);
		const $container = $(`#status-bills-container${index}`);

		if (this.sectionsOpen[index]) {
			...
			$container
				...
				.off("click")
				.on("click", (e) => {
					const id = e.currentTarget.id.replace("open-bill", "");
